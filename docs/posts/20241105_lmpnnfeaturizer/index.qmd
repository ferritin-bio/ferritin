...
One of the motivating reasons of creating `ferritin` was to be able to recreate and run protein lanaguage
...
My general strategy was to nibble at the model form both ends: begin a naive translation of the Model,
...
compact, `Tensor` representations of molecular features. I defined the `LMPNNFeatures` trait
to define the functions that I need. These functions operate on `AtomCollections`
...
One nice ergonomic crate I came accross was [strum](https://docs.rs/strum/latest/strum/).
...
how that enum is used to build up preallocated vector which we popluate and
...
are a set of decoder and encoder layers and set of deaults.  Internally,
...
can catchthe glimpse of what the model should look like.
