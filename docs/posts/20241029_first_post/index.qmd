...
This is an introduction to Ferritin, a set of crates for working with proteins.
...
```rust
// the PSEData struct is the entrypoint to Pymol's binary `pse` formay
...
// pymol and then export the results to the web via molview spec json.
//
...
I've recreated the MolViewSpec heirarchy in rust. We should then be able to translate our structure,
selection/component, and representation data from any source and export it as MSVJ. As I was initially
interested in creating a pymol converter I did make a utility in that would generate a
self-contianed HTML page with accompanying [MVSJ](https://molstar.org/viewer-docs/extensions/mvs/) json
file.
...
[vizualization in Blender](http://zachcp.org/blog/molecular_nodes/). All together I realized I was fighting or working around
different APIs to do what I needed. And so I decided - why not try writting the API that I wanted. Okay so what do I want?
...
// note heterogenous data espeially Strings
pub struct AtomInfo {
    pub resv: i32,
    pub chain: String,
    pub alt: String,
...
that most people just kind of roll their own - e.g. use vectors of homogenous data in their struct types and handle the index themself. Thats what
...
coheive and simple the code is.  I contrast this with the pymol code which has acquired multiple decades of indirections
and lots and lots of indirections. So the plan is to try to keep the high level API smooth and nice and hide away any complexity whiel
...
a dynamic background (R/Python mostly) and static language can be tricky and Rust epsceially so.
...
- Code organization. All languages have their quirks about the best way to organize your files/functions/strucuts. Rust is no different. I did downoad RustRover at some point when I wanted to move some files around and make sure all the references are preserves. Would love to see that end up in Zed at some point.
...
- Structs/Methods. As in the code organization, knowing when/how to give something its own identity didn't come natural. I started programming in functions but ende dup really likeing assocaiting methods with fewer structs as a way to minimize overhead.
- Traits. I found these a bit awkward to think about/use. When do I write them? when do I need them? I came to apprecaite them at two occasions. At one point I needed to refactor some IO code and the trait boundary was the perfect way to define the bit of code to move. Similarly as my core Stuct accumulates methods, I will want to think about grouping these funtions into traits and using thes traits to define new extension behavior.
...
inline box for small fixes, alphabetixing methods, inline tweaks, and other small, local tasks. Combined they definitely keep you in flow.
