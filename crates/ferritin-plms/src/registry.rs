...
pub enum TokenizerSpec {
    ...
    /// and so do the BERT-style protein models, over a completely different
    /// alphabet — so the file format alone does not say how to read a
    /// sequence. The [`VocabAlphabet`] does.
...
/// branched on the bare variant to pick SaProt's two-character alphabet. A
/// second `vocab.txt` model over a one-character alphabet — ProtBert was the
/// concrete case — would silently have been fed `M#Q#I#…`, made every residue
/// `<unk>`, and still passed the shape assertions (ferritin-goh.11).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum VocabAlphabet {
    ...
    SaProtPairs,
    /// One character per residue over the plain amino-acid alphabet — the
    /// BERT-style protein models, with `[PAD]`/`[UNK]`/`[CLS]`/`[SEP]`/
    /// `[MASK]` specials.
    SingleResidue,
}
